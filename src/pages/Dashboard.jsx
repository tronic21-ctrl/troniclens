        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px', marginBottom: '16px' }}>
            <img
              ...
              alt="TronicLens"
              style={{ width: '48px', height: '48px', objectFit: 'contain', flexShrink: 0, filter: settings.theme === 'light' ? 'none' : 'drop-shadow(0 0 6px #38bdf860)' }}
            />
            <div>
...
function OnboardingPopup({ onClose }) {
  ...
  const steps = [
    ...
    },
    {
      ...
      desc: 'Protocol health analysis powered by 0G Compute (Qwen2.5, TEE verified). Sentiment reflects staking behavior — not ETH price. Bullish = strong staking confidence. Insights stored permanently on 0G Storage.',
        ...
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px', marginBottom: '4px' }}>
            <img
              src={settings.theme === 'light' ? '/logos/troniclens-logo-dark.svg' : '/logos/troniclens-logo-transparent.svg'}
              alt="TronicLens"
              style={{ width: '40px', height: '40px', objectFit: 'contain', flexShrink: 0, filter: settings.theme === 'light' ? 'none' : 'drop-shadow(0 0 6px #38bdf860)' }}
            />
            <h1 style={{
          ...
          {(() => {
            const timelineData = allSnapshots
              ...
              .map(s => ({
                ...
                color: s.healthScore >= 70 ? COLORS.green : s.healthScore >= 40 ? COLORS.amber : COLORS.red,
              }))
            ...
            return (
              ...
              >
                <div style={{ padding: '14px 20px', borderBottom: `1px solid ${COLORS.cardBorder}`, backgroundColor: COLORS.chartHeader, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <span style={{ color: COLORS.textMuted, fontSize: '10px' }}>{timelineData.length} snapshots</span>
                </div>
                <div style={{ padding: '20px 24px' }}>
                  {/* Chart area */}
                  <div style={{ position: 'relative', height: '120px', marginBottom: '8px' }}>
                    ...
                    {[100, 70, 40, 0].map(val => (
                      <div key={val} style={{
                        position: 'absolute', left: 0, top: `${100 - val}%`,
                        transform: 'translateY(-50%)',
                        color: COLORS.textMuted, fontSize: '9px', fontFamily: 'monospace',
                    ...
                    {[70, 40].map(val => (
                      <div key={val} style={{
                        ...
                        top: `${100 - val}%`,
                        borderTop: `1px dashed ${val === 70 ? COLORS.green : COLORS.amber}25`,
                      }} />
                    ))}
                    {/* Bars */}
                    <div style={{ position: 'absolute', left: '36px', right: 0, top: 0, bottom: 0, display: 'flex', alignItems: 'flex-end', gap: '8px' }}>
                      {timelineData.map((d, i) => (
                        <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', height: '100%', justifyContent: 'flex-end' }}>
                          <span style={{ color: d.color, fontSize: '9px', fontWeight: 700, fontFamily: 'monospace' }}>{d.score}</span>
                          <motion.div
                            initial={{ height: 0 }}
                  ...
                  {/* X axis labels */}
                  <div style={{ paddingLeft: '36px', display: 'flex', gap: '8px' }}>
                    {timelineData.map((d, i) => (
                      <div key={i} style={{ flex: 1, textAlign: 'center' }}>
                        <span style={{ color: COLORS.textMuted, fontSize: '9px' }}>{d.date}</span>
                        <br />
                        <span style={{ color: d.color, fontSize: '8px', fontWeight: 600 }}>{d.sentiment?.slice(0, 4)}</span>
                      </div>
                    ))}
                  </div>
                </div>
