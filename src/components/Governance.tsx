...
function useCountdown(targetTimestamp: any, onComplete?: () => void) {
    ...
    >
      <motion.div
        ...
        style={{
          ...
          borderRadius: "50%",
          background:
            "radial-gradient(circle, #818cf830 0%, #818cf808 50%, transparent 70%)",
          filter: "blur(48px)",
      ...
      <motion.div
        ...
        style={{
          ...
          borderRadius: "50%",
          background:
            "radial-gradient(ellipse, #818cf815 0%, transparent 70%)",
          filter: "blur(60px)",
          willChange: "opacity",
        }}
      />
...
function PageHeader({ title, subtitle, badge, badgeColor }: PageHeaderProps) {
  const COLORS = useThemeColors();
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Connect your wallet to view proposals, check eligibility, and participate
...
function EligibilityCard({ address, onGoToStake }: EligibilityCardProps) {
  const COLORS = useThemeColors();
  const { settings } = useSettings();
  const isLight = settings.theme === "light";
  ...
  return (
    <motion.div
      ...
      style={{
        backgroundColor: COLORS.card,
        border: `1px solid ${
          eligible ? COLORS.green + "50" : COLORS.cardBorder
        }`,
        borderRadius: "12px",
        padding: "20px 24px",
        marginBottom: "24px",
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "14px" }}>
        <div
          style={{
            ...
            border: `1px solid ${
              eligible ? COLORS.green + "30" : COLORS.amber + "30"
            }`,
            display: "flex",
            alignItems: "center",
            justifyContent: "center",
        ...
        >
          {isLoading ? (
            ...
            <svg
              ...
              viewBox="0 0 24 24"
              fill="none"
              stroke={eligible ? COLORS.green : COLORS.amber}
              strokeWidth="2"
            >
              {eligible ? (
                <>
        ...
        <div>
          ...
          >
            {isLoading
              ? "—"
              : eligible
                ? `Voting Power: ${formatEth(votingPower)} ETH · ${shortenAddr(
                    address
                  )}`
                : (reason === "Tidak ada stake aktif"
        ...
        >
          <span
            style={{
              ...
              height: "6px",
              borderRadius: "50%",
              backgroundColor: isLoading ? COLORS.amber : COLORS.green,
              flexShrink: 0,
            }}
          />
...
function CreateProposalForm({ eligible, onSuccess }: CreateProposalFormProps) {
  const COLORS = useThemeColors();
                ...
                <motion.button
                  ...
                  style={{
                    ...
                    backgroundColor: description.trim()
                      ...
                      : COLORS.card,
                    border: `1px solid ${
                      description.trim() ? COLORS.cyan + "60" : COLORS.cardBorder
                    }`,
                    borderRadius: "8px",
...
function ProposalCard({
  ...
  }, [isSuccess, refetchInfo, refetchStatus, refetchVote]);
  ...
  const status = (statusData as string) || "Loading...";
      ...
      <div style={{ display: "flex", gap: "16px", flexWrap: "wrap" }}>
        ...
        >
          ...
          <span
            onClick={() =>
              window.open(
                `https://eth-sepolia.blockscout.com/address/${proposer}`,
                "_blank"
              )
            }
            style={{
        ...
        {deadlineDate && (
          <span style={{ color: COLORS.textMuted, fontSize: "12px" }}>
            Deadline:{" "}
            {deadlineDate.toLocaleString("id-ID", { timeZone: "UTC" })} UTC
          </span>
        )}
    ...
    return (
      ...
      >
        <p style={{ color: COLORS.textMuted, fontSize: "14px" }}>
          No proposals yet.{" "}
          {eligible ? "Be the first to create one!" : "Stake ETH to participate."}
        </p>
      </motion.div>
    );
  }
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        {!isConnected ? (
          ...
          <>
            <EligibilityCard
              address={address}
              onGoToStake={() => onItemClick("stake-action")}
            />
            <CreateProposalForm
              eligible={eligible}
