...
function ChartTooltip({ active, payload, label, tab, range }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '8px 0 0', height: '240px' }}>
        {loading && tab !== 'TVL' ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: '12px' }}>
            ...
            <p style={{ color: C.dim, fontSize: '13px', textAlign: 'center' }}>CoinGecko rate limit reached</p>
            {retryIn > 0 ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <div style={{ width: '6px', height: '6px', borderRadius: '50%', backgroundColor: C.amber }} />
                <span style={{ color: C.amber, fontSize: '12px', fontFamily: 'monospace', fontWeight: 600 }}>Retrying in {retryIn}s...</span>
              </div>
            ) : (
              <button onClick={() => fetchPriceData(range)} style={{ padding: '6px 16px', background: `${C.cyan}15`, border: `1px solid ${C.cyan}40`, borderRadius: '8px', color: C.cyan, fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}>Retry now</button>
            )}
          </div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={priceData} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey="price" stroke={lineColor} strokeWidth={2} fill="url(#priceGrad)" dot={false} activeDot={{ r: 4, fill: lineColor, strokeWidth: 0 }} isAnimationActive={false} />
            </AreaChart>
          </ResponsiveContainer>
        ) : tab === 'Volume' ? (
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={volumeData} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
              ...
              <Bar dataKey="volume" fill={C.purple} fillOpacity={0.8} radius={[2, 2, 0, 0]} isAnimationActive={false} />
            </BarChart>
          </ResponsiveContainer>
        ) : tab === 'TVL' ? (
          tvlData.length === 0 ? (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
              <motion.span animate={{ opacity: [0.3, 1, 0.3] }} transition={{ duration: 1.5, repeat: Infinity }} style={{ color: C.dim, fontSize: '13px' }}>Loading TVL...</motion.span>
            </div>
          ) : (
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={tvlData} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="globalTVL" name="ETH Ecosystem" stroke={C.cyan} strokeWidth={2} fill="url(#globalTVLGrad)" dot={false} isAnimationActive={false} />
                <Area type="monotone" dataKey="tronicTVL" name="TronicLens" stroke={C.green} strokeWidth={2} fill="url(#tronicTVLGrad)" dot={false} isAnimationActive={false} />
              </AreaChart>
            </ResponsiveContainer>
          )
        ) : null}
      ...
      {/* ── FULLSCREEN OVERLAY (DENGAN MENUNYA LENGKAP) ── */}
      <AnimatePresence>
        {isFullscreen && (
          <motion.div
          ...
          >
            ...
            {/* ── BARIS MENU UTAMA: TABS + TIMEFRAMES (INI YANG AWALNYA HILANG) ── */}
            <div style={{
              padding: '10px 16px',
              borderBottom: `1px solid ${C.border}`,
              backgroundColor: '#040a14',
            ...
            {/* Fullscreen Chart Container — Menggunakan Overlay Absolute */}
            <div style={{ flex: 1, padding: '8px 0', display: 'flex', flexDirection: 'column', minHeight: 0, position: 'relative', width: '100%' }}>
              ...
              {loading && (!priceData || priceData.length === 0) && (
                <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#040a14', zIndex: 10 }}>
                  <motion.span animate={{ opacity: [0.3, 1, 0.3] }} transition={{ duration: 1.5, repeat: Infinity }} style={{ color: C.dim, fontSize: '13px' }}>Loading chart...</motion.span>
                </div>
              )}
              ...
              {tab === 'Price' && chartType === 'line' && (
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={priceData} margin={{ top: 8, right: 20, left: 0, bottom: 0 }}>
                    ...
                    <Tooltip content={<ChartTooltip tab="Price" range={range}/>} cursor={{ stroke: C.cyan, strokeWidth: 1, strokeDasharray: '4 4' }}/>
                    <Area type="monotone" dataKey="price" stroke={lineColor} strokeWidth={2} fill="url(#fsGrad)" dot={false} activeDot={{ r: 5, fill: lineColor, strokeWidth: 0 }} isAnimationActive={false}/>
                  </AreaChart>
                </ResponsiveContainer>
              )}
              ...
              {tab === 'TVL' && (
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={tvlData} margin={{ top: 8, right: 20, left: 0, bottom: 0 }}>
                    ...
                    <Tooltip content={<ChartTooltip tab="TVL" range={range}/>} cursor={{ stroke: C.cyan, strokeWidth: 1, strokeDasharray: '4 4' }}/>
                    <Area type="monotone" dataKey="globalTVL" name="ETH Ecosystem" stroke={C.cyan} strokeWidth={2} fill="url(#fsTVL1)" dot={false} isAnimationActive={false}/>
                    <Area type="monotone" dataKey="tronicTVL" name="TronicLens" stroke={C.green} strokeWidth={2} fill="url(#fsTVL2)" dot={false} isAnimationActive={false}/>
                  </AreaChart>
                </ResponsiveContainer>
              )}
            </div>
            ...
            {/* Fullscreen Footer */}
            <div style={{ padding: '8px 16px', borderTop: `1px solid ${C.border}`, display: 'flex', justifyContent: 'space-between', backgroundColor: '#040a14' }}>
              <span style={{ color: C.dim, fontSize: '10px' }}>
                {tab === 'TVL' ? 'ETH Ecosystem TVL via DeFiLlama · TronicLens TVL via The Graph' : 'Historical data via CoinGecko · Live price via Chainlink'}
              </span>
